...
class Render
  ...
  def svg_line(params, color = :black)
    <<~LINE.gsub(/\s+/, " ").strip
      <line x1="#{params[:x1]}" y1="#{params[:y1]}"
      x2="#{params[:x2]}"
      y2="#{params[:y2]}"
      style="stroke:#{color.to_s}; stroke-width:1"/>
    LINE
  end

  def svg_circle(params, color = :black)
    <<~CIRCLE.gsub(/\s+/, " ").strip
      <circle cx="#{params[:cx]}"
      cy="#{params[:cy]}"
      r="#{params[:r]}"
      fill="white" style="stroke:#{color.to_s}; stroke-width:1"/>
    CIRCLE
  end

  def svg_ellipse(params, color = :black)
    <<~ELLIPSE.gsub(/\s+/, " ").strip
      <ellipse cx="#{params[:cx]}"
      cy="#{params[:cy]}"
      rx="#{params[:rx]}"
      ry="#{params[:ry]}"
      style="stroke:#{color.to_s}; stroke-width:1" fill="white"/>
    ELLIPSE
  end

  def svg_text(params, color = :black)
    <<~TEXT.gsub(/\s+/, " ").strip
      <text x="#{params[:x]}"
      y="#{params[:y]}"
      font-family="arial" font-size="#{params[:font_size]}"
      fill="#{color.to_s}" style="writing-mode:
